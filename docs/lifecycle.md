...
Panic внутри callback преобразуется в `*HookPanicError`: фаза (`OnStart` или
`OnStop`), исходный индекс регистрации, значение panic и стек доступны через
`errors.As`. Если значение panic — ошибка, она также доступна через
`errors.Is`/`errors.As`. Panic в `OnStart` запускает обычный rollback; panic
в `OnStop` объединяется с другими ошибками очистки и не мешает попытке остановить
остальные активные hooks. Переход состояния завершается; однократность cleanup
и поведение повторных вызовов сохраняются, как и для возвращаемых ошибок.
